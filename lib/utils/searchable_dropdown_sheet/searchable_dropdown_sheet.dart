...
class SearchableDropdownSheet extends StatefulWidget {
  ...
  @override
  State<SearchableDropdownSheet> createState() => _SearchableDropdownSheetState();
}
...
class _SearchableDropdownSheetState extends State<SearchableDropdownSheet> {
  ...
  String _displayText() {
    ...
    return widget.selectedValue![widget.nameKey]?.toString() ?? widget.hintText ?? "";
  }
            ...
            return StatefulBuilder(
              builder: (context, setModalState) {
                ...
                final filtered = q.isEmpty
                    ? list
                    : list.where((m) {
                        final n = m[widget.nameKey]?.toString().toLowerCase() ?? '';
                        return n.contains(q);
                      }).toList();
                return Column(
  ...
  Widget build(BuildContext context) {
    final border = OutlineInputBorder(
      ...
      borderSide: widget.borderSide ?? BorderSide(color: Color(AppColors.whiteGrey), width: 1.0),
    );
    ...
    final contentPadding = EdgeInsets.only(
      ...
      top: widget.height != null ? verticalPadding : (base.top > 0 ? base.top : 12),
      bottom: widget.height != null ? verticalPadding : (base.bottom > 0 ? base.bottom : 12),
    );
    final child = InkWell(
      onTap: _openSheet,
