...
/// تسجيل آخر وقت وتاريخ لفتح التطبيق
/// - آخر وقت فتح عام (مع أو بدون إنترنت)
/// - آخر وقت فتح مع وجود إنترنت
Future<void> _recordAppOpenTime() async {
  try {
    final now = DateTime.now();
    final timestamp = now.toIso8601String();
    final dateTimeString = '${now.year}-${now.month.toString().padLeft(2, '0')}-${now.day.toString().padLeft(2, '0')} ${now.hour.toString().padLeft(2, '0')}:${now.minute.toString().padLeft(2, '0')}:${now.second.toString().padLeft(2, '0')}';
    
    // تسجيل آخر وقت فتح عام (مع أو بدون إنترنت)
    await CacheHelper.setString(
      key: 'lastAppOpenTime',
      value: timestamp,
    );
    await CacheHelper.setString(
      key: 'lastAppOpenDateTime',
      value: dateTimeString,
    );
    
    debugPrint('📱 App opened at: $dateTimeString (ISO: $timestamp)');
    
    // التحقق من وجود إنترنت
    try {
      final isOnline = await ConnectionsService.isOnline();
      
      if (isOnline) {
        // تسجيل آخر وقت فتح مع وجود إنترنت
        await CacheHelper.setString(
          key: 'lastAppOpenTimeWithInternet',
          value: timestamp,
        );
        await CacheHelper.setString(
          key: 'lastAppOpenDateTimeWithInternet',
          value: dateTimeString,
        );
        
        debugPrint('🌐 App opened with internet at: $dateTimeString (ISO: $timestamp)');
      } else {
        debugPrint('📴 App opened without internet at: $dateTimeString');
      }
    } catch (e) {
      debugPrint('⚠️ Error checking internet connection: $e');
      // في حالة الخطأ، لا نحدث lastAppOpenTimeWithInternet
    }
  } catch (e, stackTrace) {
    debugPrint('⚠️ Error recording app open time: $e');
...
void main() async {
  ...
  // Register global error handlers (Telegram error service doesn't need initialization)
  registerErrorHandlers();
  
  await CacheHelper.init();
  
  // تسجيل آخر وقت وتاريخ لفتح التطبيق
  await _recordAppOpenTime();
