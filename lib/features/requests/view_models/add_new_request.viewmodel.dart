...
class AddNewRequestViewModel extends ChangeNotifier {
  ...
  void _getRequestTypes({required BuildContext context}) {
    try {
      ...
      debugPrint("TYPE IS --> ${UserSettingConst.generalSettingsModel!.requestTypes}");
      ...
      userBalance.forEach((requestId, val) {
        ...
        if (max != null &&
          ...
          if (max == -1 || ((userBalanceRequestType?['take'] ?? 0) < max)) {
            ...
            debugPrint("requestsTypes is --> $requestsTypes");
  ...
  void _calcDuration({required BuildContext context}) {
    ...
    debugPrint("isSameDay --> ${isSameDay}");
    debugPrint("halfDay --> ${halfDay}");
    debugPrint("type --> ${type?.toLowerCase().trim()}");
  ...
  bool isWeekendOrHolidayDateFromString(BuildContext context, String dateString) {
    ...
    if (!weekendDays.contains('variable')) {
      ...
      debugPrint("holidays is --> $holidays");
      debugPrint("canUseHolidays is --> ${user2Settings?.canUseHolidays}");
    // Check holidays if user can use holidays
    if (user2Settings?.canUseHolidays == true && holidays != null && holidays.isNotEmpty) {
      debugPrint("holidays is --> $holidays");
      debugPrint("canUseHolidays is --> ${user2Settings?.canUseHolidays}");
      debugPrint("weekendDays is --> $weekendDays");
      debugPrint("date is --> $date");
      debugPrint("date.weekday is --> ${date.weekday}");
      debugPrint("weekdaysMap is --> $weekdaysMap");
      debugPrint("weekdaysMap[date.weekday] is --> ${weekdaysMap[date.weekday]}");
      debugPrint("weekendDays.contains(weekdaysMap[date.weekday]) is --> ${weekendDays.contains(weekdaysMap[date.weekday])}");
      debugPrint("weekendDays.contains(weekdaysMap[date.weekday]) is --> ${weekendDays.contains(weekdaysMap[date.weekday])}");
  ...
  void checkHalfDate(BuildContext context, date) {
    ...
    notifyListeners();
    debugPrint('$dateString is weekend? $isWeekend');
  }
  void _getDateDifferenceWithoutWeekendsAndOfficailHolidays({required BuildContext context}) {
   ...
   debugPrint("WEEK HOLIDAY IS --->${user2Settigns!.weekend}");
    ...
    // Second: Subtracting Holidays if the Current user can use holidays
      debugPrint("holidays is --> $holidays");
      debugPrint("canUseHolidays is --> ${user2Settigns.canUseHolidays}");
  ...
  String getHoursOrDayesStringDependsOnRequestType() {
    debugPrint("HOUSRS US --> ${ (reqType as String?)?.trim().toLowerCase() }");
  ...
  Future<void> createNewRequest({required BuildContext context}) async {
    try {
      ...
      debugPrint("isAttachingFile $attachingFile");
