...
class SmartCardViewModel extends ChangeNotifier {
  ...
  Future<void> loadMyCompanies(BuildContext context) async {
    _setLoading(true);
    try {
      final res = await SmartCardRepo.getMyCompanies(context);
      final data = res['data'];
      if (data is List) {
  ...
  Future<void> loadCompanyProfile(BuildContext context) async {
    ...
    _setLoading(true);
    try {
      final res = await SmartCardRepo.getCompanyProfile(context, companyId: selectedCompanyId!);
      final data = res['data'];
      if (data is Map) {
  ...
  Future<void> loadCompanyEmployees(BuildContext context) async {
    ...
    _setLoading(true);
    try {
      final res = await SmartCardRepo.getCompanyEmployees(context, companyId: selectedCompanyId!);
      final data = res['data'];
      if (data is List) {
  ...
  Future<void> loadTemplates(BuildContext context) async {
    _setLoading(true);
    try {
      final res = await SmartCardRepo.getTemplates(context);
      final data = res['data'];
      if (data is List) {
  ...
  Future<void> loadSmartCardScreen(BuildContext context) async {
    ...
    try {
      ...
      try {
        final profileRes = await SmartCardRepo.getEmployeeProfile(context);
  ...
  String getCompanyPublicUrl() {
    ...
    return SmartCardRepo.getCompanyProfilePublicUrl(slug.toString());
  }
  ...
  String getEmployeePublicUrl(Map<String, dynamic> emp) {
    ...
    return SmartCardRepo.getEmployeeProfilePublicUrl(slug.toString());
  }
  ...
  Future<bool> createEmployeeProfile(BuildContext context,
    ...
    try {
      ...
      await SmartCardRepo.createEmployee(
        context,
  ...
  Future<bool> addEmployeeToCompany(BuildContext context, {required String name}) async {
    ...
    _setLoading(true);
    try {
      await SmartCardRepo.addEmployeeToCompany(
        context,
  ...
  Future<bool> updateCompanyTemplate(BuildContext context, int templateId) async {
    ...
    _setLoading(true);
    try {
      await SmartCardRepo.updateCompanyTemplate(
        context,
        companyId: selectedCompanyId!,
